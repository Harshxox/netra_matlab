...
`;
  ...
  function renderScanResult(rec) {
    ...
    box.innerHTML =
      ...
      + (layers.length
          ? '<p class="nt-h">Evidence</p>'
            ...
            + '<img class="nt-img" id="nt-layer-img" src="' + url(rec.images[layers[0][0]]) + '" alt="">'
    ...
    $$('.nt-layers button', box).forEach((b) => {
      b.addEventListener('click', () => {
        ...
        b.classList.add('on');
        if (img) img.src = url(rec.images[b.dataset.layer]);
      });
    });
  ...
  function renderRecord(rec) {
    ...
    if (pv && (imgs.evidence || imgs.gradcam || imgs.original)) {
      pv.src = url(imgs.evidence || imgs.gradcam || imgs.original);
    }
  ...
  function swapRetinaCanvas(rec) {
    ...
    const show = (n) => {
      ...
      if (src) { host.src = url(src); host.style.display = 'block'; }
